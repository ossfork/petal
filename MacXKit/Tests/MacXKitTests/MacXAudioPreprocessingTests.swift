...
func trimSilenceRemovesLeadingAndTrailingSections() async throws {
    ...
    let outputURL = try await MacXAudioTrimClient.liveValue.trimSilence(inputURL, 0.01)
...
func speedUpReducesDurationAtExpectedRate() async throws {
    ...
    let outputURL = try await MacXAudioSpeedClient.liveValue.speedUp(inputURL, 1.25)
    defer { try? FileManager.default.removeItem(at: outputURL) }
...
func transcriptionClientUsesTrimAndSpeedDependenciesBeforeMLX() async throws {
    ...
    let output = try await withDependencies {
        ...
        $0.macXAudioTrimClient.trimSilence = { url, _ in
            await recorder.append("trim")
            return url
        }
        $0.macXAudioSpeedClient.speedUp = { url, rate in
            await recorder.append("speed:\(String(format: "%.2f", rate))")
            return url
        }
    } operation: {
        try await MacXTranscriptionClient.liveValue.transcribe(audioURL, .mini3b, .verbatim, nil)
    }
