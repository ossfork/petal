...
public class VoxtralForConditionalGeneration: Module, LanguageModel {
    ...
    private func dumpSwiftAudioFeatures(_ inputFeatures: MLXArray) {
        let debugDir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("voxtral_debug", isDirectory: true)
        try? FileManager.default.createDirectory(at: debugDir, withIntermediateDirectories: true)

        let dumpFile = debugDir
            .appendingPathComponent("swift_audio_features_dump.txt")
            .path
        ...
        let simpleFile = debugDir
            .appendingPathComponent("swift_audio_features_simple.txt")
            .path
    ...
    private func loadPythonAudioFeatures() -> MLXArray? {
        let pythonAudioFile = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("voxtral_debug", isDirectory: true)
            .appendingPathComponent("python_audio_features_simple.txt")
            .path
