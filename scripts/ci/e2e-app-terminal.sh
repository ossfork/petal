...
TRIGGER_MODE="hotkey"
LAUNCH_MODE=""
TEXT="macx app e2e verification sentence"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      echo "Usage: $(basename "$0") --app <path/to/macx.app> [--trigger hotkey|deeplink] [--launch terminal|open] [--text <sentence>] [--wait-timeout <seconds>]" >&2
      exit 1
      ;;
  esac
done
...
APP_BIN="$APP_PATH/Contents/MacOS/macx"
if [[ ! -x "$APP_BIN" ]]; then
...
count_instances() {
  pgrep -x macx | wc -l | tr -d '[:space:]'
}

kill_all_macx() {
  pkill -x macx 2>/dev/null || true
  sleep 1
  if [[ "$(count_instances)" != "0" ]]; then
    pkill -9 -x macx 2>/dev/null || true
    sleep 1
  fi
}
...
assert_single_instance() {
  ...
  if [[ "$count" != "1" ]]; then
    echo "Expected exactly one macx instance at ${label}, found $count" >&2
    exit 1
  fi
}
...
echo "Ensuring no running macx instances before E2E"
kill_all_macx
echo "instances_before_launch=$(count_instances)"

echo "Starting log stream"
/usr/bin/log stream --style compact --level debug \
  --predicate '(process == "macx") || (subsystem == "com.optimalapps.macx")' \
  > "$LOG_FILE" 2>&1 &
LOG_PID=$!
sleep 2

if [[ "$LAUNCH_MODE" == "terminal" ]]; then
  echo "Launching macx from terminal executable: $APP_BIN"
  "$APP_BIN" > "$APP_STDOUT_FILE" 2>&1 &
  APP_PID=$!
else
  echo "Launching macx through LaunchServices: open $APP_PATH"
  open "$APP_PATH"
fi
sleep 4
...
if [[ "$(count_instances)" != "0" ]]; then
  echo "macx is still running after shutdown cleanup" >&2
  exit 1
fi
